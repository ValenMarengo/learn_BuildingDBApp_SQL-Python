# Tkinter GUI(Graphical User Interface) for CRUD(Create, Read, Update, Delete [Basic functions]) operations on a student database using PostgreSQL

from tkinter import *
from tkinter import ttk, messagebox #submodule
import psycopg2
...
def delete_data():
    ...
    parameters = (student_id,) # "," for create (1-element tuple)
    run_query(query, parameters)
    messagebox.showinfo("Information", "Data deleted successfully.")
    refresh_treeview()
 
#new code added ..........................................
def run_query(query, parameters=()): # receive query and parameters as tuple
    conn = psycopg2.connect(dbname='sample', 
                        user='va', 
                        password='admin123', 
                        host='localhost',
                        port='5432'
    ...
    try:
        ...
        if query.lower().startswith("select"): # Convert query to lowercase and check if it starts with "select"
            query_result = cur.fetchall() # Gets all query results at once as a list of tuples, Each tuple represents a row.
        conn.commit()
...
def refresh_treeview():
    ...
    for item in tree.get_children(): # get all rows showing in Treeview
        tree.delete(item)
    # Re-fetch and display the updated data
    records = run_query("SELECT * FROM students;") 
    # records = list of tuple [(name, address, age, number),(),...]
...
frame.grid(row=0,column=0,padx=10,pady=10,sticky="ew") # frame grid layout manager
